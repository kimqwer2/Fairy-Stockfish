...
namespace {
  ...
  Value value_draw(const Position& pos) {
    int noise = 2 * (pos.this_thread()->nodes & 1) - 1;

    if (pos.is_janggi_modern())
        return Value(24 * ((pos.material_counting_result() > VALUE_DRAW) - (pos.material_counting_result() < VALUE_DRAW)) + noise);

    return VALUE_DRAW + Value(noise);
  }
